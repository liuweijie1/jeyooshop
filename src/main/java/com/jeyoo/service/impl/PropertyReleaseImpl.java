...
public class PropertyReleaseImpl implements IPropertyReleaseService{
	...
	public void upOrDownProperty(Long propertyid,Boolean isadminoffline) {
		if(null == propertyid) throw new JeyooException(ExceptionEnum.BAD_REQUEST);
		...
		if(result != null) {
			...
			if(num == 0) {
				throw new JeyooException(ExceptionEnum.PROPERTY_UPDATE_ERROR);
			}
		}
	}
	...
	public int releaseApply(Long propertyId,Boolean apply) {
		...
		if(result != null) {
			...
			int num = propertyMapper.updateProperty(result);
			if(num == 0) {
				throw new JeyooException(ExceptionEnum.PROPERTY_APPROVAL_ERROR);
			}
		}
