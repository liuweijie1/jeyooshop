...
import com.jeyoo.mapper.SysPropertyCoinsetMapper;
import com.jeyoo.pojo.SysPropertyCoinset;
import com.jeyoo.service.ISysPropertyCoinsetService;
...
public class SysPropertyCoinsetServiceImpl implements ISysPropertyCoinsetService{
	...
	public int insertCoinset(List<SysPropertyCoinset> coinset) {
		if(null != coinset) {
			for (int i = 0; i < coinset.size(); i++) {
				...
				if(num == 0) {
					throw new JeyooException(ExceptionEnum.PROPERTY_COINSET_INSERT_ERROR);
				}
			}
		}
		return 0;
	...
	public void upOrDownCoinset(Integer setid) {
		if(null == setid) throw new JeyooException(ExceptionEnum.BAD_REQUEST);
		...
		if(null != coinset) {
			...
			if(num == 0) {
				throw new JeyooException(ExceptionEnum.PROPERTY_COINSET_LOWER_ERROR);
			}
		}
	}
